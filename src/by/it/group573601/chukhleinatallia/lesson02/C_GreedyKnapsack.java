...
//макс стоимость
    double calc(File source) throws FileNotFoundException {
        ...
        Arrays.sort(items);//цуну по убыв
        ...
        for (i=0; i<items.length;i++) {//пока не дойдем до конца
            if (items[i].weight <= WeightBackpack) {//если вес меньше
                ...
                totalUnitPrice = totalUnitPrice+ items[i].cost;//+ценц
                CostBackpack--;
            }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        File f=new File(root+"by/it/group573601/chukhleinatallia/lesson02/greedyKnapsack.txt");
