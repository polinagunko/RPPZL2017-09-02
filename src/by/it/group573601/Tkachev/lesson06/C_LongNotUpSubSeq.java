...
public class C_LongNotUpSubSeq {
    ...
    int getNotUpSeqSize(InputStream stream) throws FileNotFoundException {
        ...
        int ansver = 0;
        for (int i = 0; i < length; i++) {
            ansver = Math.max(ansver, buff[i]);
        }

        int[] indexOfAnsver = new int[ansver];

        int index = 0;

        for (int i = 1; i < length; i ++) {
            if (buff[i] > buff[index]) {
                index = i;
            }
        }
        ...
        int i = ansver-1;
        while (index >= 0){
            indexOfAnsver[i] = index + 1;
            i--;
            index = prev[index];
        }
        System.out.println(Arrays.toString(indexOfAnsver));
        //!!!!!!!!!!!!!!!!!!!!!!!!!     КОНЕЦ ЗАДАЧИ     !!!!!!!!!!!!!!!!!!!!!!!!!
        return ansver;
    }
