...
public class A_Huffman {
    ...
    abstract class Node implements Comparable<Node> {
        ...
        //только через его версии InternalNode и LeafNode
        private final int frequence; //частота символов

        //генерация кодов на корневом узле
        //после построения дерева один раз
    ...
    private class InternalNode extends Node {
        ...
        //внутренних узлов не существет без наличия обоих детей
    ...
    private class LeafNode extends Node {
        ...
        @Override
        void fillCodes(String code) {
            //решение дошло до листа, рекурсия закончена
            //запоминает его в индексе для поиска кода по символу.
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        {
            if (count.putIfAbsent(s.charAt(k),  1)!=null)
                count.put(s.charAt(k),  count.get(s.charAt(k))+1);
        }
        ...
        //У родителя ветви(частоты) детей складываются.
        Node a,b;
        while (true)
        {
        ...
        for (char letter: s.toCharArray()) sb.append(codes.get(letter));
        //.....
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        File f = new File(root + "by/it/group573601/chukhleinatallia/lesson03/dataHuffman.txt");
