package by.it.group573601.michmel.lesson01.lesson03;
...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for (int i = 0; i < s.length(); i++)
            if (count.putIfAbsent(s.charAt(i), 1) != null)//вставка, если нет такого ключа(возвращает тру)
                count.put(s.charAt(i), count.get(s.charAt(i)) + 1);//вставка вместо ключа, кот. существует
        ...
        Node first, next;
        while (true) {
            first = priorityQueue.poll();//возвращвет с удление элемент из нчальной очереди
            next = priorityQueue.poll();
            priorityQueue.add(new InternalNode(first, next));
        ...
        priorityQueue.peek().fillCodes(result.toString());//возвращает верхний элемент не удалаяя из стека
        for (char letter : s.toCharArray())
            result.append(codes.get(letter));//добавляет в строку
