...
public class C_LongNotUpSubSeq {
    ...
    int getNotUpSeqSize(InputStream stream) throws FileNotFoundException {
        ...
        for (int i = 0; i < length; i++) {
            ...
            prev[i] = -1;
            for (int j = 0; j < i; j++)
                if (massive[j] >= massive[i] && buff[j] + 1 > buff[i]) { //Тут невозрастающая
                    buff[i]=buff[i]+1;
                    prev[i] = j;
                }
        }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        InputStream stream = new FileInputStream(root + "by/it/group573601/Stephanovich/lesson06/dataC.txt");        C_LongNotUpSubSeq instance = new C_LongNotUpSubSeq();
