package by.it.group573601.arobey.lesson03;
...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        String s = scann.next();
        Map<Character, Integer> countMap = new HashMap<>();//ассоциативный контейнер
        ...
        PriorityQueue<Node> priorQue = new PriorityQueue<>();

        for (Character key : countMap.keySet()){
            int value = countMap.get(key);
            priorQue.add(new LeafNode(value,key));
        }

        int size = priorQue.size();

        for (int i = size + 1; i < 2 * size; i++){
            Node left = priorQue.poll();
            Node right = priorQue.poll();
            priorQue.add(new InternalNode(left,right));
        }

        StringBuilder sb = new StringBuilder();


        priorQue.peek().fillCodes(sb.toString());
