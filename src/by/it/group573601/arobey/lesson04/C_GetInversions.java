...
public class C_GetInversions {
    ...
    private int[] merge(int[] mII, int[] mJJ){
        int i =0;
        int j = 0;
        int[] mKK = new int[mII.length + mJJ.length];
        for (int k =0; k< mKK.length; k++){
            if (j==mJJ.length || (i<mII.length && mII[i] <= mJJ[j])){
                mKK[k] = mII[i++];}
            else
            {
                mKK[k] = mJJ[j++];
                inversions+=(mII.length-i);
            }
        }
        return mKK;
    }
