        ...
        //комментарии от проверочного решения сохранены для подсказки, но вы можете их удалить.
         ...
         Arrays.sort(events);                 //hint: сортировка Arrays.sort обеспечит скорость алгоритма
             ...
             while(i < events.length-1) {      //пока есть незарегистрированные события
                 ...
                 result.add(events[i]);          //и запомним время старта видеокамеры
                 double k = events[i] + workDuration;   //вычислим момент окончания работы видеокамеры
                 while (events[i] < k) {         //и теперь пропустим все покрываемые события
                     i = i + 1;                 // за время до конца работы, увеличивая индекс
                 }
             }
