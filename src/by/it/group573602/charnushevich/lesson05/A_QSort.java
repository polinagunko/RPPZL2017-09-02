...
public class A_QSort {
    ...
    private int partition(Segment[] a,int low,int high){
        ...
        for (int i = low+1;i<=high;i++){
            if (a[i].compareTo(x) <= 0){
                m++;
                Segment temp = a[i];        //сначала ставим сегмент, который начинается раньше
                a[i] = a[m];
                a[m] = temp;
            }
        }
    ...
    //сортируем сегменты относительно их начала(start) и продолжительности
    private void qsort(Segment[] foo,int low,int high){
        if(low < high){
            int m = partition(foo,low,high);
            qsort(foo,low,m-1);
            qsort(foo,m+1,high);
        }
    }
    ...
    int[] getAccessory(InputStream stream) throws FileNotFoundException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if(start > stop){       //меняем местами начало и конец, если они приходят в обратном порядке
                int temp = start;
                start = stop;
                stop = temp;
            }
        ...
        for(Segment segment:segments){
            if(segment.stop < 0)
                ...
                result[segment.stop] = segmentNum;
        }
